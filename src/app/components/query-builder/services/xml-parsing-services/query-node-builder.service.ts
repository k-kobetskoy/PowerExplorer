...
export class QueryNodeBuilderService {
  ...
  buildQueryNode(): IQueryNodeBuildResult {
    ...
    let queryNode = new QueryNode(QueryNodeData.GetNodeName(this.tag.tagName), this.attributeFactoryResolver);

    if (this.attributes.length > 0) {
      const attributeFactory = this.attributeFactoryResolver.getAttributesFactory(QueryNodeData.GetNodeName(this.tag.tagName));
