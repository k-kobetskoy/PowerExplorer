...
// Create config.js content for production (file:// protocol paths)
function createConfigContent(isProduction = false) {
  // For Electron production builds, we need to use relative paths
  const redirectPrefix = isProduction ? './' : 'http://localhost:4200/';
  ...
  POST_LOGOUT_REDIRECT_URI: '${redirectPrefix}',
  IS_ELECTRON: ${isProduction}
};`;
}

// Create configuration.json content
function createConfigJsonContent(isProduction = false) {
  // For Electron production builds, we need to use relative paths
  const redirectPrefix = isProduction ? './' : 'http://localhost:4200/';
  ...
  return JSON.stringify({
    msal: {
      auth: {
        ...
        postLogoutRedirectUri: redirectPrefix,
...
try {
  // Write production config files with production settings
  fs.writeFileSync(configJsPath, createConfigContent(true));
  console.log(`Environment config.js written to ${configJsPath}`);

  fs.writeFileSync(configJsonPath, createConfigJsonContent(true));
...
try {
  ...
  fs.writeFileSync(devConfigJsPath, createConfigContent(false));
  console.log(`Development config.js written to ${devConfigJsPath}`);

  fs.writeFileSync(devConfigJsonPath, createConfigJsonContent(false));
